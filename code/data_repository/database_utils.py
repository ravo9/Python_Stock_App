...
SQL_CREATE_TABLE_CASH_FLOW_STATEMENT = '''CREATE TABLE IF NOT EXISTS cash_flow_statement (company_name TEXT, date TEXT, net_cash_flow REAL, PRIMARY KEY (company_name, date))'''
SQL_CREATE_TABLE_INCOME_STATEMENT = '''CREATE TABLE IF NOT EXISTS income_statement (company_name TEXT, date TEXT, interest_expense REAL, PRIMARY KEY (company_name, date))'''
SQL_CREATE_TABLE_BALANCE_SHEET = '''CREATE TABLE IF NOT EXISTS balance_sheet (company_name TEXT, date TEXT, total_debt REAL, PRIMARY KEY (company_name, date))'''
SQL_CREATE_TABLE_SHARES_AMOUNT = '''CREATE TABLE IF NOT EXISTS SHARES_AMOUNT (ticker TEXT, date TEXT, shares_amount REAL, PRIMARY KEY (ticker, date))'''
SQL_CREATE_TABLE_SHARE_PRICE = '''CREATE TABLE IF NOT EXISTS SHARE_PRICE (ticker TEXT, date TEXT, share_price REAL, PRIMARY KEY (ticker, date))'''
SQL_CREATE_TABLE_SHARE_PRICES_IN_PERIOD = '''CREATE TABLE IF NOT EXISTS SHARE_PRICES_IN_PERIOD (ticker TEXT, start_date TEXT, end_date TEXT, data TEXT, PRIMARY KEY (ticker, start_date, end_date))'''
...
def save_financial_statements_data(statement_type, financial_data, ticker):
    for report in financial_data:
        ...
        if statement_type == 'cash_flow_statement': save_data_to_database(SQL_CREATE_TABLE_CASH_FLOW_STATEMENT, SQL_INSERT_CASH_FLOW_STATEMENT, ticker, date, report['freeCashFlow'])
        elif statement_type == 'income_statement': save_data_to_database(SQL_CREATE_TABLE_INCOME_STATEMENT, SQL_INSERT_INCOME_STATEMENT, ticker, date, report['interestExpense'])
        elif statement_type == 'balance_sheet': save_data_to_database(SQL_CREATE_TABLE_BALANCE_SHEET, SQL_INSERT_BALANCE_SHEET, ticker, date, report['totalDebt'])
    ...
    with sl.connect(DATABASE_PATH) as con:
        ...
        if are_reports_stored:
            try:
                ...
                return reports
            except Exception as e: print(f"Error in get_stored_{statement_type}_statements_if_available: Request failed: {ticker} - {e}")
        return None
